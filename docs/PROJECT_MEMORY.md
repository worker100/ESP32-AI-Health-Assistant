...
## 49. 2026-03-14 P0-v1 算法融合升级（Accuracy + Realtime Balance）
- 本次主线目标：
  - 在不牺牲“实时观感”的前提下，提高 HR/SpO2 有效值可信度，减少动作与弱信号导致的飘值。
- 已并入主程序（`firmware/src/main.cpp`）：
  - HR：
    - 引入 `algo-HR` 新鲜度窗口（`kAlgoHrFreshMs`）；
    - `checkForBeat` 路新增“与最近 `algo-HR` 一致性校验”；
    - 基于 `MPU` 的低运动/强运动门控，强运动低置信时抑制可疑拍点；
    - HR 平滑改为“质量+运动”自适应（高质量低运动更快，低质量更稳）。
  - SpO2：
    - fallback 路新增“限次短时补洞”（`kSpo2FallbackMaxConsecutive`）；
    - fallback 仅在低运动、质量达标、置信度达标时允许；
    - SpO2 主值与显示平滑改为自适应。
- 设计定位：
  - 这是“低风险结构增强”，并非医疗级标定算法；
  - 仍以毕设演示稳定性和可解释性为优先。

## 50. 2026-03-15 开题报告需求对照结论（Gap Review）
- 依据文档：
  - `D:\Projects\ESP32 Projects\_extract\kaiti_report_full.txt`
  - 说明：该导出文件存在明显重复段，需求提取以“研究目的/创新点”去重后为准。
- 对照结论（截至 2026-03-15）：
  - 已基本完成：
    - 多传感器核心链路（HR/SpO2/Temperature/Fall）；
    - AI 语音问答链路（采集/上行/回复/TTS/打断）；
    - OLED + 本地告警联动；
    - 主程序多模块并行运行与异常自愈。
  - 仍需补齐（开题目标未完全闭环）：
    - 便携供电实装与验收记录（`18650 + TP4056 + Boost`）；
    - “精准采集”量化证明（与对照设备的误差统计报告）；
    - 系统级性能测试报告归档（准确性/稳定性/交互延迟/便携性）。
- 当前建议优先级：
  1. 固化对照测试流程并产出误差统计文档；
  2. 完成电池供电链路实装与续航记录；
